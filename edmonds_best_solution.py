...
def find_best_route( distanceMatrix , odd_nodes, edge_list, depot=0):
    ...
    return path, length
def find_odd_nodes_and_edgelist(distanceMatrix):
    ...
    for i in range(num_nodes):
        # 計算第 i 個節點的非零連接數（即排除自身和未連接的節點）
        connections = np.count_nonzero(distanceMatrix[i, :])
        if connections % 2 != 0:
            odd_nodes.append(i)
    
    # 找出所有存在的邊
    for i in range(num_nodes):
        for j in range(i + 1, num_nodes):  # 只考慮上三角矩陣，避免重複計算
            if distanceMatrix[i, j] != 0:
                edgelist.append((i, j))

    return odd_nodes, edgelist
def resolution(distanceMatrix):
    np.fill_diagonal(distanceMatrix, False)
    odd_nodes, edgelist = find_odd_nodes_and_edgelist(distanceMatrix)
...
if __name__ == "__main__":
    import cpp_env
    import time
    cp = cpp_env.ChinesePostman()
    state = cp.reset()
    distanceMatrix = cp.map
